...
@Table(name = "couriers")
@NamedQueries({
        @NamedQuery(name = "Courier.findByLoginOrPhoneOrEmail",
                query = "select c from Courier c where c.login=:login " +
                        "or c.phone=:login or c.email=:login"),
        @NamedQuery(name = "Courier.findAll",
                query = "select c from Courier c")
})
public class Courier implements Authentication, Serializable {
    @Id
    @GeneratedValue
    private Long id;
